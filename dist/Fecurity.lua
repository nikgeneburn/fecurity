...
fromScale(1,1),ZIndex=2},i))j.Root=k for l=0,e-1 do local m=((l*53)%100)/100 local n=m^1.8 local o=(l*23+math.floor(l/3)*11)%100 local p=4+n*12+(l%5)*0.18 local q=18-n*11+(l*7%5)local r=(l*0.47)%q
local s=math.round((((l*29)%120)-60)*(0.35+n*1.25))local t=0.35+n*0.5 local u=l%2==0 and 180 or-180 local v=(c.New('Frame',{Name='Flake'..tostring(l),BackgroundColor3=Color3.new(1,1,1),
BackgroundTransparency=1-t,BorderSizePixel=0,Size=UDim2.fromOffset(p,p),ZIndex=2},k))c.Corner(v,math.ceil(p/2))table.insert(j.Flakes,{Node=v,LeftPercent=o,Duration=q,Delay=r,Drift=s,Spin=u})end j.
Connection=b.RenderStepped:Connect(function(l)j:Step(l)end)return j end function d.Step(h,i)if not h.Visible then return end local j=h.Root.AbsoluteSize if j.X<=0 or j.Y<=0 then return end h.Elapsed+=
i for k,l in ipairs(h.Flakes)do local m=((h.Elapsed+l.Delay)%l.Duration)/l.Duration local n=(l.LeftPercent/100)*j.X+m*l.Drift local o=(f+m*(g-f))*j.Y l.Node.Position=UDim2.fromOffset(n,o)l.Node.
Rotation=m*l.Spin end end function d.SetVisible(h,i)h.Visible=i h.Root.Visible=i end function d.Destroy(h)if h.Connection then h.Connection:Disconnect()end if h.Root then h.Root:Destroy()end end
return d end function a.x()local b=a.cache.x if not b then b={c=__modImpl()}a.cache.x=b end return b.c end end do local __modImpl=function()local b={}b.__index=b b.Height=0 function b.New(c,d,e)local
f=setmetatable({Window=c,Root=d,Title=e.Title or'Fecurity',Subtitle=e.Subtitle or''},b)f:Apply()return f end function b.Apply(c)c.Root:SetAttribute('FecurityTitle',c.Title)c.Root:SetAttribute(
'FecuritySubtitle',c.Subtitle)end function b.SetTitle(c,d,e)c.Title=d if e~=nil then c.Subtitle=e end c:Apply()end return b end function a.y()local b=a.cache.y if not b then b={c=__modImpl()}a.cache.y
=b end return b.c end end do local __modImpl=function()local b=a.j()local c={}function c.FadeIn(d)d.Visible=true d.BackgroundTransparency=1 b.Play(d,0.15,{BackgroundTransparency=0})end function c.
FadeOut(d)local e=b.Play(d,0.15,{BackgroundTransparency=1})e.Completed:Once(function()if d and d.Parent then d.Visible=false end end)end function c.PanelIn(d,e)local f=d.Position d.Visible=true d.
Position=f+UDim2.fromOffset(0,7)d.BackgroundTransparency=1 task.delay(e or 0,function()if d and d.Parent then b.Play(d,0.16,{Position=f,BackgroundTransparency=0})end end)end function c.PanelOut(d,e)
local f=d.Position task.delay(e or 0,function()if not d or not d.Parent then return end local g=b.Play(d,0.12,{Position=f+UDim2.fromOffset(0,7),BackgroundTransparency=1},Enum.EasingStyle.Quad,Enum.
EasingDirection.In)g.Completed:Once(function()if d and d.Parent then d.Visible=false d.Position=f d.BackgroundTransparency=0 end end)end)end return c end function a.z()local b=a.cache.z if not b then
b={c=__modImpl()}a.cache.z=b end return b.c end end do local __modImpl=function()local b=a.n()local c={}function c.Vertical(d,e)local f=b.List(d,Enum.FillDirection.Vertical,e or 0)f.
...
i.Step j:SetValue(math.clamp(u,i.Min,i.Max))end j:TrackConnection(n.InputBegan:Connect(function(r)if r.UserInputType==Enum.UserInputType.MouseButton1 then q=true SetFromMouse()end end))j:
TrackConnection(p.InputBegan:Connect(function(r)if r.UserInputType==Enum.UserInputType.MouseButton1 then q=true end end))j:TrackConnection(b.InputEnded:Connect(function(r)if r.UserInputType==Enum.
UserInputType.MouseButton1 then q=false end end))j:TrackConnection(b.InputChanged:Connect(function(r)if q and r.UserInputType==Enum.UserInputType.MouseMovement then SetFromMouse()end end))j.
...
Accent p.BackgroundColor3=r.Accent end)j:SetValue(j.Value,false)return j end function g.TrackConnection(h,i)table.insert(h.Connections,i)return i end function g.DisconnectInputs(h)for i,j in ipairs(h.
Connections)do j:Disconnect()end table.clear(h.Connections)if h.DestroyConnection then h.DestroyConnection:Disconnect()h.DestroyConnection=nil end end function g.SetValue(h,i,j)local k=tonumber(i)or h
.Options.Min k=math.clamp(k,h.Options.Min,h.Options.Max)h:Commit(k,j)local l=(k-h.Options.Min)/math.max(h.Options.Max-h.Options.Min,1)h.Fill.Size=UDim2.new(l,0,0,2)h.Knob.Position=UDim2.new(l,0,0,1)h.
ValueLabel.Text=Format(h.Options,k)end function g.Destroy(h)h:DisconnectInputs()c.Destroy(h)end return g end function a.D()local b=a.cache.D if not b then b={c=__modImpl()}a.cache.D=b end return b.c
end end do local __modImpl=function()local b=a.B()local c=a.n()local d=a.s()local e=a.t()local f=a.j()local g={}g.__index=g setmetatable(g,{__index=b})local Contains=function(h,i)for j,k in ipairs(h)
...
h.Window.Theme local j=h.Window.Root.AbsolutePosition local k=h.Box.AbsolutePosition local l=22 local m=math.max(l,math.min(#h.Options.Values,7)*l)local n=(c.New('Frame',{Name='DropdownMenu',
BackgroundColor3=i.Control,BackgroundTransparency=0.15,BorderSizePixel=0,ClipsDescendants=true,Position=UDim2.fromOffset(k.X-j.X,k.Y-j.Y+h.Box.AbsoluteSize.Y+1),Size=UDim2.fromOffset(h.Box.
...
Options.Multi and type(i)~='table'then i={i}end h:Commit(i,j)h.ValueLabel.Text=Display(i)end function g.SetValues(h,i,j)h.Options.Values=i or{}local k=j or h.Value if h.Options.Multi then local l=if
type(k)=='table'then k else{k}local m={}for n,o in ipairs(l)do if Contains(h.Options.Values,o)then table.insert(m,o)end end h:SetValue(m,false)elseif not Contains(h.Options.Values,k)then h:SetValue(h.
Options.Values[1]or'None',false)else h:SetValue(k,false)end if h.Menu then local l=h.Menu h.Menu=nil h.OptionRows=nil l:Destroy()h:OpenMenu()else h:RefreshRows()end end function g.RefreshRows(h)if not
h.OptionRows then return end local i=h.Window.Theme for j,k in ipairs(h.OptionRows)do local l=if h.Options.Multi then Contains(h.Value,k.Option)else h.Value==k.Option if k.Indicator then k.Indicator.
BackgroundColor3=i.Accent k.Indicator.BackgroundTransparency=l and 0 or 1 end if k.IndicatorStroke then k.IndicatorStroke.Color=l and i.Accent or i.Muted end k.Label.TextColor3=l and i.Accent or i.
Muted k.Row.BackgroundColor3=l and i.ControlHover or i.Control end if h.Menu then h.Menu.BackgroundColor3=i.Control end end function g.Destroy(h)h:CloseMenu(true)if h.DestroyConnection then h.
...
Destroy(i)end return h end function a.H()local b=a.cache.H if not b then b={c=__modImpl()}a.cache.H=b end return b.c end end do local __modImpl=function()local b=a.k()local c={}c.Templates={'#ff0000',
'#5f90f6','#d8f368','#e1ae24','#3ad4bb','#fee2eb','#f25dab','#ffffff'}c.HueSequence=ColorSequence.new({ColorSequenceKeypoint.new(0,Color3.fromRGB(255,0,0)),ColorSequenceKeypoint.new(0.17,Color3.
fromRGB(255,255,0)),ColorSequenceKeypoint.new(0.33,Color3.fromRGB(0,255,0)),ColorSequenceKeypoint.new(0.5,Color3.fromRGB(0,255,255)),ColorSequenceKeypoint.new(0.67,Color3.fromRGB(0,0,255)),
ColorSequenceKeypoint.new(0.83,Color3.fromRGB(255,0,255)),ColorSequenceKeypoint.new(1,Color3.fromRGB(255,0,0))})function c.ToColorAlpha(d)if typeof(d)=='Color3'then return d,nil end if type(d)==
'table'then local e,f=c.ToColorAlpha(d.Color or d.Value)return e,tonumber(d.Alpha)or f end if type(d)=='string'then local e,f=b.ParseWithAlpha(d)return e or b.Hex(d),f end return Color3.new(1,1,1),nil
end function c.AddGradient(d,e,f,g)local h=Instance.new('UIGradient')h.Color=e if f then h.Transparency=f end h.Rotation=g or 0 h.Parent=d return h end return c end function a.I()local b=a.cache.I if
not b then b={c=__modImpl()}a.cache.I=b end return b.c end end do local __modImpl=function()local b=game:GetService('UserInputService')local c=a.B()local d=a.n()local e=a.s()local f=a.t()local g=a.k()
...
l.Hex then l.Hex.BackgroundColor3=q.Control l.Hex.TextColor3=q.Text end end)l:SetValue(l.Value,false)return l end function i.DisconnectPopup(j)for k,l in ipairs(j.PopupConnections)do l:Disconnect()end
table.clear(j.PopupConnections)end function i.Track(j,k)table.insert(j.PopupConnections,k)return k end function i.ClosePicker(j)j:DisconnectPopup()j.DragTarget=nil if j.Popup then j.Popup:Destroy()j.
Popup=nil j.Apply=nil j.Hex=nil j.Preview=nil j.Square=nil j.SquareCursor=nil j.HueCursor=nil j.AlphaBar=nil j.AlphaCursor=nil end end function i.TogglePicker(j)if j.Popup then j:ClosePicker()else j:
OpenPicker()end end function i.CreateColorSquare(j,k,l)local m=(d.New('Frame',{Name='ColorSquare',BackgroundColor3=Color3.fromHSV(j.Hue,1,1),BorderSizePixel=0,Position=UDim2.fromOffset(10,40),Size=
UDim2.fromOffset(182,182),ZIndex=221},k))j.Square=m local n=(d.New('Frame',{Name='White',BackgroundColor3=Color3.new(1,1,1),BorderSizePixel=0,Size=UDim2.fromScale(1,1),ZIndex=222},m))h.AddGradient(n,
ColorSequence.new(Color3.new(1,1,1)),NumberSequence.new({NumberSequenceKeypoint.new(0,0),NumberSequenceKeypoint.new(1,1)}))local o=(d.New('Frame',{Name='Black',BackgroundColor3=Color3.new(0,0,0),
BorderSizePixel=0,Size=UDim2.fromScale(1,1),ZIndex=223},m))h.AddGradient(o,ColorSequence.new(Color3.new(0,0,0)),NumberSequence.new({NumberSequenceKeypoint.new(0,1),NumberSequenceKeypoint.new(1,0)}),90
)local p=(d.New('Frame',{Name='Cursor',BackgroundColor3=l.Accent,BorderSizePixel=0,Size=UDim2.fromOffset(7,7),ZIndex=224},m))d.Corner(p,4)d.Stroke(p,l.Text,0.4,1)j.SquareCursor=p j:Track(m.InputBegan:
Connect(function(q)if q.UserInputType==Enum.UserInputType.MouseButton1 then j.DragTarget='square'j:UpdateFromSquare()end end))end function i.CreateHueBar(j,k,l)local m=(d.New('Frame',{Name='Hue',
BackgroundColor3=Color3.new(1,1,1),BorderSizePixel=0,Position=UDim2.fromOffset(10,232),Size=UDim2.fromOffset(182,10),ZIndex=221},k))h.AddGradient(m,h.HueSequence)j.HueBar=m local n=(d.New('Frame',{
Name='HueCursor',BackgroundColor3=l.Accent,BorderSizePixel=0,Size=UDim2.fromOffset(7,7),ZIndex=222},m))d.Corner(n,4)d.Stroke(n,l.Text,0.4,1)j.HueCursor=n j:Track(m.InputBegan:Connect(function(o)if o.
UserInputType==Enum.UserInputType.MouseButton1 then j.DragTarget='hue'j:UpdateFromHue()end end))end function i.CreateAlphaBar(j,k,l)local m=(d.New('Frame',{Name='Alpha',BackgroundColor3=j.Value,
BorderSizePixel=0,Position=UDim2.fromOffset(10,248),Size=UDim2.fromOffset(182,10),ZIndex=221},k))h.AddGradient(m,ColorSequence.new(Color3.new(1,1,1)),NumberSequence.new({NumberSequenceKeypoint.new(0,1
),NumberSequenceKeypoint.new(1,0)}))j.AlphaBar=m local n=(d.New('Frame',{Name='AlphaCursor',BackgroundColor3=l.Accent,BorderSizePixel=0,Size=UDim2.fromOffset(7,7),ZIndex=222},m))d.Corner(n,4)d.Stroke(
n,l.Text,0.4,1)j.AlphaCursor=n j:Track(m.InputBegan:Connect(function(o)if o.UserInputType==Enum.UserInputType.MouseButton1 then j.DragTarget='alpha'j:UpdateFromAlpha()end end))end function i.
CreateTemplates(j,k)local l=j.Window.Theme for m,n in ipairs(h.Templates)do local o=(d.New('TextButton',{Name='Template'..tostring(m),AutoButtonColor=false,BackgroundColor3=g.Hex(n),BorderSizePixel=0,
Position=UDim2.fromOffset(10+(m-1)*24,302),Size=UDim2.fromOffset(16,16),Text='',ZIndex=221},k))d.Corner(o,8)d.Stroke(o,l.Text,0.8,1)j:Track(o.MouseButton1Click:Connect(function()j:SetValue(g.Hex(n))
end))end end function i.OpenPicker(j)local k=j.Window.Theme local l=j.Window.Root.AbsolutePosition local m=j.Swatch.AbsolutePosition local n=(d.New('Frame',{Name='ColorPickerPopup',BackgroundColor3=k.
...
elseif j.DragTarget=='alpha'then j:UpdateFromAlpha()end end))j:Track(b.InputEnded:Connect(function(r)if r.UserInputType==Enum.UserInputType.MouseButton1 then j.DragTarget=nil end end))j:
CreateTemplates(n)j:UpdateVisuals()end function i.UpdateFromSquare(j)if not j.Square then return end local k=b:GetMouseLocation()local l=j.Square.AbsolutePosition local m=j.Square.AbsoluteSize j.
Saturation=math.clamp((k.X-l.X)/math.max(m.X,1),0,1)j.Brightness=1-math.clamp((k.Y-l.Y)/math.max(m.Y,1),0,1)j:SetValue(Color3.fromHSV(j.Hue,j.Saturation,j.Brightness))end function i.UpdateFromHue(j)if
not j.HueBar then return end local k=b:GetMouseLocation()local l=j.HueBar.AbsolutePosition local m=j.HueBar.AbsoluteSize j.Hue=math.clamp((k.X-l.X)/math.max(m.X,1),0,1)j:SetValue(Color3.fromHSV(j.Hue,
j.Saturation,j.Brightness))end function i.UpdateFromAlpha(j)if not j.AlphaBar then return end local k=b:GetMouseLocation()local l=j.AlphaBar.AbsolutePosition local m=j.AlphaBar.AbsoluteSize j:
SetAlpha((k.X-l.X)/math.max(m.X,1))end function i.SetAlpha(j,k,l)j.Alpha=math.clamp(tonumber(k)or j.Alpha or 1,0,1)if j.Flag then j.Library.FlagManager:Set(j.Flag..'.Alpha',j.Alpha)end if l~=false
then j.Library:RunCallback(j.Options.Text or'Color',j.Callback,j.Value,j.Alpha)end j:UpdateVisuals()end function i.UpdateVisuals(j)local k=j.Value j.Swatch.BackgroundColor3=k j.Swatch.
BackgroundTransparency=1-j.Alpha if j.Preview then j.Preview.BackgroundColor3=k j.Preview.BackgroundTransparency=1-j.Alpha end if j.Square then j.Square.BackgroundColor3=Color3.fromHSV(j.Hue,1,1)end
if j.SquareCursor then j.SquareCursor.Position=UDim2.new(j.Saturation,-4,1-j.Brightness,-4)end if j.HueCursor then j.HueCursor.Position=UDim2.new(j.Hue,-3,0,1)end if j.AlphaBar then j.AlphaBar.
BackgroundColor3=k end if j.AlphaCursor then j.AlphaCursor.Position=UDim2.new(j.Alpha,-3,0,1)end if j.Hex then j.Hex.Text=j.Alpha<0.999 and g.ToRgba(k,j.Alpha)or g.ToHex(k)end end function i.SetValue(
j,k,l)local m,n=h.ToColorAlpha(k)if n~=nil then j.Alpha=math.clamp(n,0,1)end if j.Flag then j.Library.FlagManager:Set(j.Flag..'.Alpha',j.Alpha)end j.Hue,j.Saturation,j.Brightness=Color3.toHSV(m)j:
Commit(m,l)j:UpdateVisuals()end function i.Destroy(j)j:ClosePicker()if j.AlphaFlag then j.Library.FlagManager:Unregister(j.AlphaFlag)end if j.DestroyConnection then j.DestroyConnection:Disconnect()j.
DestroyConnection=nil end c.Destroy(j)end return i end function a.J()local b=a.cache.J if not b then b={c=__modImpl()}a.cache.J=b end return b.c end end do local __modImpl=function()local b=a.B()local
c=a.n()local d=a.s()local e=a.t()local f=a.r()local g=a.j()local h={}h.__index=h setmetatable(h,{__index=b})local i={{Id='First',Color=Color3.fromRGB(72,25,20),Stroke=Color3.fromRGB(131,53,68)},{Id=
'Second',Color=Color3.fromRGB(75,54,27),Stroke=Color3.fromRGB(162,148,112)},{Id='Third',Color=Color3.fromRGB(35,61,23),Stroke=Color3.fromRGB(101,149,94)},{Id='Fourth',Color=Color3.fromRGB(12,62,90),
Stroke=Color3.fromRGB(50,115,145)},{Id='Ignore',Color=Color3.fromRGB(29,30,36),Stroke=Color3.fromRGB(69,71,77)}}local j={{Id='Head',X=73,Y=12,W=34,H=48},{Id='Body',X=52,Y=60,W=78,H=116},{Id='LeftArm',
X=18,Y=72,W=42,H=118},{Id='RightArm',X=122,Y=72,W=42,H=118},{Id='LeftLeg',X=54,Y=170,W=38,H=110},{Id='RightLeg',X=91,Y=170,W=38,H=110}}local k={{Name='Head',X=75,Y=14,W=30,H=38,Round=18},{Name='Neck',
X=82,Y=52,W=16,H=16,Round=4},{Name='Torso',X=56,Y=66,W=68,H=104,Round=4},{Name='LeftArm',X=24,Y=78,W=24,H=108,Round=4},{Name='RightArm',X=132,Y=78,W=24,H=108,Round=4},{Name='LeftLeg',X=58,Y=170,W=28,H
=108,Round=4},{Name='RightLeg',X=94,Y=170,W=28,H=108,Round=4}}function h.CreateImageFallback(l,m)local n=(c.New('Frame',{Name='PreviewFallback',BackgroundTransparency=1,BorderSizePixel=0,Position=
UDim2.fromOffset(17,8),Size=UDim2.fromOffset(178,274),ZIndex=30},l.Root))l.FallbackParts={}for o,p in ipairs(k)do local q=(c.New('Frame',{Name=p.Name,BackgroundColor3=m.Muted,BackgroundTransparency=
0.52,BorderSizePixel=0,Position=UDim2.fromOffset(p.X,p.Y),Size=UDim2.fromOffset(p.W,p.H),ZIndex=30},n))c.Corner(q,p.Round)table.insert(l.FallbackParts,q)end l:BindTheme(function(o)for p,q in ipairs(l.
FallbackParts)do q.BackgroundColor3=o.Muted end end)end function h.New(l,m)m.Default=m.Default or{}local n=b.New(l,m,390)setmetatable(n,h)local o=n.Window.Theme n.RegionState=table.clone(n.Value)n.
Overlays={}local p=(c.New('ImageLabel',{Name='PreviewImage',BackgroundTransparency=1,Position=UDim2.fromOffset(17,8),Size=UDim2.fromOffset(178,274),ScaleType=Enum.ScaleType.Fit,ZIndex=31},n.Root))if
not f.Apply(p,n.Library.AssetCache,'HitboxPreview')then p.Visible=false n:CreateImageFallback(o)end n.Image=p for q,r in ipairs(j)do local s=(c.New('TextButton',{Name=r.Id,AutoButtonColor=false,
BackgroundColor3=o.Accent,BackgroundTransparency=1,BorderSizePixel=0,Position=UDim2.fromOffset(17+r.X,8+r.Y),Size=UDim2.fromOffset(r.W,r.H),Text='',ZIndex=34},n.Root))n.Overlays[r.Id]=s s.
...
GetConfigs()if#g==0 then return{'None'}end return g end e.Text=e.Text or'Config'e.Hint=e.Hint or'Select saved config'e.Flag=e.Flag or'config.selected'e.Values=ReadValues()e.Default=e.Default or e.
...
Create('Slider',h,i)end function g.AddDropdown(h,i)return f.Create('Dropdown',h,i)end function g.AddList(h,i)return h:AddDropdown(i)end function g.AddButton(h,i)return f.Create('Button',h,i)end
function g.AddTextbox(h,i)return f.Create('Textbox',h,i)end function g.AddBox(h,i)return h:AddTextbox(i)end function g.AddKeybind(h,i)return f.Create('Keybind',h,i)end function g.AddBind(h,i)return h:
AddKeybind(i)end function g.AddColor(h,i)return f.Create('ColorPicker',h,i)end function g.AddHitboxPreview(h,i)return f.Create('HitboxPreview',h,i)end function g.AddCountdown(h,i)return f.Create(
'Countdown',h,i)end function g.AddConfigList(h,i)return f.Create('ConfigList',h,i)end return g end function a.P()local b=a.cache.P if not b then b={c=__modImpl()}a.cache.P=b end return b.c end end do
local __modImpl=function()local b=a.n()local c=a.A()local d=a.m()local e=a.P()local f={}f.__index=f function f.New(g,h)local i=g.Window local j=setmetatable({Tab=g,Window=i,Index=h,Sections={}},f)
local k=(b.New('Frame',{Name='Column'..tostring(h),BackgroundColor3=i.Theme.Surface,BorderSizePixel=0,Position=UDim2.fromOffset(0,d.PanelTop),Size=UDim2.fromOffset(d.PanelWidth,d.PanelHeight),ZIndex=
21},i.Content))j.Stroke=b.Stroke(k,i.Theme.Border,0,1)j.Root=k local l=(b.New('ScrollingFrame',{Name='Scroll',Active=true,BackgroundTransparency=1,BorderSizePixel=0,Position=UDim2.fromOffset(0,0),Size
=UDim2.fromScale(1,1),ScrollBarThickness=4,ScrollBarImageColor3=i.Theme.Accent,CanvasSize=UDim2.fromOffset(0,0),AutomaticCanvasSize=Enum.AutomaticSize.Y,ZIndex=22},k))b.Padding(l,15,16,15,16)local m=c
.Vertical(l,0)j.Scroll=l j.Layout=m j:ApplyLayout(h,1)i:RegisterThemeBinding(function(n)k.BackgroundColor3=n.Surface j.Stroke.Color=n.Border l.ScrollBarImageColor3=n.Accent end)return j end function f
.ApplyLayout(g,h,i)g.Index=h local j=g.Window.TargetSize or UDim2.fromOffset(d.WindowSize.X,d.WindowSize.Y)local k=j.X.Offset>0 and j.X.Offset or d.WindowSize.X local l=j.Y.Offset>0 and j.Y.Offset or
d.WindowSize.Y local m,n=d.PanelLayout(h,i,k)g.Root.Position=UDim2.fromOffset(m,d.PanelTop)g.Root.Size=UDim2.fromOffset(n,d.PanelHeightFor(l))end function f.AddSection(g,h,i)local j=e.New(g,h,i)table.
insert(g.Sections,j)return j end return f end function a.Q()local b=a.cache.Q if not b then b={c=__modImpl()}a.cache.Q=b end return b.c end end do local __modImpl=function()local b=a.z()local c=a.Q()
local d={}d.__index=d function d.New(e,f,g,h)local i=setmetatable({Window=e,Name=f,Icon=g.Icon or string.lower(f),Index=h,Columns={},Active=false},d)i.Button=e.Sidebar:AddButton(i)return i end
function d.AddColumn(e)local f=c.New(e,#e.Columns+1)table.insert(e.Columns,f)e:RelayoutColumns()f.Root.Visible=e.Active return f end function d.RelayoutColumns(e)local f=#e.Columns for g,h in ipairs(e
.Columns)do h:ApplyLayout(g,f)end end function d.SetActive(e,f)e.Active=f if e.Button then e.Button:SetActive(f)end for g,h in ipairs(e.Columns)do if f then b.PanelIn(h.Root,(g-1)*0.025)else b.
PanelOut(h.Root,(g-1)*0.012)end end end function d.RefreshTheme(e)if e.Button then e.Button:RefreshTheme()end end return d end function a.R()local b=a.cache.R if not b then b={c=__modImpl()}a.cache.R=
b end return b.c end end do local __modImpl=function()local b=a.n()local c={}c.__index=c function c.New(d)local e=setmetatable({},c)e.Content=(b.New('Frame',{Name='Content',BackgroundTransparency=1,
BorderSizePixel=0,Position=UDim2.fromOffset(0,0),Size=UDim2.fromScale(1,1),ZIndex=20},d))e.Overlay=(b.New('Frame',{Name='Overlay',BackgroundTransparency=1,BorderSizePixel=0,Position=UDim2.fromOffset(0
...
local o=a.T()local p=a.V()local q={}q.__index=q local ScaleSize=function(r,s)return UDim2.new(r.X.Scale*s,math.floor(r.X.Offset*s),r.Y.Scale*s,math.floor(r.Y.Offset*s))end function q.New(r,s)local t=
setmetatable({Library=r,Options=s,TargetSize=s.Size or UDim2.fromOffset(f.WindowSize.X,f.WindowSize.Y),Theme=e.Resolve(s.Theme or'Dark',s.Accent),Tabs={},ActiveTab=nil,Visible=true,MenuKey=i.
Normalize(s.MenuKey or s.ToggleKey or Enum.KeyCode.Insert),Dropdown=nil,ThemeBindings={}},q)local u=Instance.new('ScreenGui')u.Name='Fecurity'u.IgnoreGuiInset=true u.ResetOnSpawn=false u.
ZIndexBehavior=Enum.ZIndexBehavior.Sibling d.AttachGui(u)t.Gui=u local v=(g.New('Frame',{Name='Canvas',BackgroundTransparency=1,BorderSizePixel=0,Size=UDim2.fromScale(1,1),ZIndex=1},u))t.Canvas=v t.
SnowLayer=k.New(t,v)local w=(g.New('Frame',{Name='Window',AnchorPoint=Vector2.new(0.5,0.5),Position=UDim2.fromScale(0.5,0.5),Size=t.TargetSize,BackgroundColor3=t.Theme.Surface,BorderSizePixel=0,ZIndex
=10},v))t.RootStroke=g.Stroke(w,t.Theme.Border,0,1)t.Root=w t.Topbar=l.New(t,w,s)t.TabContainer=n.New(w)t.Content=t.TabContainer.Content t.Overlay=t.TabContainer.Overlay t.Notifications=(g.New('Frame'
,{Name='Notifications',BackgroundTransparency=1,BorderSizePixel=0,AnchorPoint=Vector2.new(1,0),Position=UDim2.new(1,-16,0,16),Size=UDim2.fromOffset(250,400),ZIndex=400},v))g.List(t.Notifications,Enum.
FillDirection.Vertical,8)t.Sidebar=j.New(t)t.UnbindDrag=h.Attach(w,w)t.InsertConnection=b.InputBegan:Connect(function(x,y)if y then return end if i.IsActivation(x,t.MenuKey)then t:Toggle()end end)t:
